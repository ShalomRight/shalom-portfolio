...
// ── DETAIL PAGE LAYOUT ────────────────────────────────────────
// Maps each project to the correct template component.
// Assigned explicitly in portfolio.data.ts; drives dynamic routing.
//
//  "detail-tree"     → portfolio-details-3     (deep narrative, case study)
//  "showcase-detail" → portfolio-showcase-details (deep, visual-strong)
//  "detail-video"    → portfolio-details-video  (video-primary project)
//  "detail-one"      → portfolio-details-1      (many images, no carousel)
//  "detail-two"      → portfolio-details-2      (many images + carousel)

export type DetailPageLayout =
  | "detail-tree"
  | "showcase-detail"
  | "detail-video"
  | "detail-one"
...
export interface Portfolio {
  ...
  // Individual project pages — each gets its own URL at /work/[slug]
  projects: PortfolioProject[];

  // Category gallery pages — each is a scrollable showcase grid at /work/[type]
...
//  /work                    → Main portfolio grid (core projects only)
//  /work/[slug]             → Individual ProjectPage (standard | deep)
//  /work/logos              → GalleryPage — logos (Portfolio Random layout)
//  /work/social-kits        → GalleryPage — social media kits
//  /work/motion             → GalleryPage — motion graphics
